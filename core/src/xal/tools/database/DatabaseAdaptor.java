...
public abstract class DatabaseAdaptor {
	...
	/**
	 * set an SQL Array to the database, since MySQL doesn't support SQL Array, so we use JSON to encode an Java array to String to store in database
	...
	/**
	 * get an array from the database, since MySQL doesn't support SQL Array, so we use JSON to decode String from database to an Java array
	 * @param record The ResultSet which hold the values fetched from database
	 * @param valueType The identified String for the values
	 * @return An Java array
	 * @throws SQLException
	 */
